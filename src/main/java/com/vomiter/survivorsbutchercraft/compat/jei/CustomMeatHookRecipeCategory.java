...
public class CustomMeatHookRecipeCategory implements IRecipeCategory<CustomMeatHookRecipe> {
    public static final RecipeType<CustomMeatHookRecipe> TYPE = RecipeType.create(SurvivorsButchercraft.MODID, "hook", CustomMeatHookRecipe.class);
    private final Component localizedName = Component.translatable("Butchercraft.jei.hook");
    private final IDrawable icon;

    public CustomMeatHookRecipeCategory(IGuiHelper guiHelper) {
        this.icon = guiHelper.createDrawableIngredient(VanillaTypes.ITEM_STACK, new ItemStack((ItemLike)ButchercraftItems.HOOK_BLOCK_ITEM.get()));
    }

    public RecipeType<CustomMeatHookRecipe> getRecipeType() {
        return TYPE;
    }
    ...
    public void setRecipe(IRecipeLayoutBuilder builder, CustomMeatHookRecipe recipe, IFocusGroup focuses) {
        int count = recipe.getHookRecipe().tools().size();
        int offset = 2;
        int width = 16 + offset;
        int placementH = 0;
        ...
        int placementW = 0;
        int c = 0;
        builder.addSlot(RecipeIngredientRole.INPUT, this.getWidth() / 2 - 8, 40).addIngredients(recipe.getHookRecipe().carcass());

        for(AnimatedRecipeItemUse a : recipe.getHookRecipe().tools()) {
    ...
    public void draw(CustomMeatHookRecipe recipe, IRecipeSlotsView recipeSlotsView, GuiGraphics guiGraphics, double mouseX, double mouseY) {
