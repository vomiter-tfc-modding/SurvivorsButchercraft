...
public class ChanceResult {
    ...
    public ItemStack rollOutput(RandomSource random, int fortuneLevel) {
        ...
        for(int roll = 0; roll < this.stack.getCount(); ++roll) {
            if ((double)random.nextFloat() > (double)this.chance + fortuneBonus) {
                --outputAmount;
            }
        }
    ...
    public static ChanceResult deserialize(JsonElement je) {
        if (!je.isJsonObject()) {
            ...
            ItemStack itemstack = new ItemStack(ForgeRegistries.ITEMS.getValue(ResourceLocation.tryParse(itemId)), count);
            if (json.has("nbt")) {
                try {
